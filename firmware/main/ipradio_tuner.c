...
/* Регистры блоков RDS.
 *
 * СВЕРЕНО С ДАТАШИТОМ v1.1, стр. 12 (docs/ref/RDA5807M_datasheet.pdf).
 * Прежняя пометка «не проверено» снята: адреса совпали, гадать
 * не пришлось. Оценки ошибок блоков A и B лежат отдельно, в 0BH,
 * и теперь используются при разборе. */
...
/* Регистр 04H */
#define R04_DE        (1u << 11)  /* 0 = 75 мкс, 1 = 50 мкс             */
...
/* Регистр 0BH.
 * Проверено по даташиту v1.1, стр. 11: уровень сигнала в старших
 * семи битах, ниже - признак блока E и оценки ошибок блоков A и B. */
#define R0B_ABCD_E    (1u << 4)   /* 1 = в 0CH..0FH лежит блок E       */
#define R0B_BLERB_MASK  0x0003u   /* ошибки блока B: 3 = неисправимые   */
#define R0B_BLERA_MASK  0x000Cu
...
/* Теневая копия 04H. Раньше в этот регистр писались нули, то есть
 * предыскажения оставались 75 мкс - американская норма. В России
 * и Европе 50 мкс, и с чужим значением звук выходит подчёркнуто
 * ярким и шипящим на верхах. Даташит RDA5807M v1.1, регистр 04H,
 * бит 11 (docs/ref/RDA5807M_datasheet.pdf, стр. 9). */
static uint16_t       s_r04 = R04_DE;
...
esp_err_t ipradio_tuner_set_volume(uint8_t logical)
{
    ...
    /* Ноль громкости у этого чипа - НЕ «очень тихо».
     *
     * Даташит v1.1, стр. 10: «When 0000, output mute and output
     * impedance is very large». То есть нулевое поле VOLUME само,
     * помимо бита DHIZ, уводит выход в высокоимпедансное состояние
     * и отцепляет тюнер от пассивного сумматора.
     *
     * Округление давало ноль на всей нижней части шкалы: логические
     * 1…3 превращались в 0. Человек, убавивший звук почти до нуля,
     * получал не тихий эфир, а полную тишину с отключённым источником.
     * Поэтому всё, что больше нуля, получает как минимум единицу,
     * а настоящая тишина остаётся ровно на нуле шкалы - и там
     * отключение выхода как раз кстати. */
    if (logical > 0 && vol == 0) {
        vol = 1;
    }

    /* Правим ТОЛЬКО поле громкости. Рядом в этом же регистре лежит
     * порог автопоиска SEEKTH (биты 11:8), и запись громкости целым
     * словом обнулила бы его - автопоиск перестал бы находить
     * станции. Известная ловушка этого чипа. */
...
static void tuner_task(void *arg)
{
    ...
    for (;;) {
        ...
        /* RDS. Пускаем разбор по флагу «группа готова».
         *
         * Синхронизацию декодера (RDSS) РАНЬШЕ ТРЕБОВАЛИ обязательно,
         * и это было опасно: даташит (стр. 11) прямо пишет, что RDSS
         * «available only in RDS Verbose mode», а чем этот режим
         * включается — в документе не сказано нигде. Если бы он
         * не включился, имена станций не появились бы никогда,
         * и причину искали бы в декодере.
         *
         * Поэтому RDSS теперь только к сведению, а отсев мусора
         * делают две настоящие проверки ниже.
         *
         * Первая: при ABCD_E в регистрах лежит блок E режима RBDS,
         * а не блоки A–D. Разбирать его как группу нельзя.
         *
         * Вторая: BLERB — оценка ошибок блока B, а именно в нём
         * номер группы и номер пары знаков. Значение 3 означает
         * «шесть и больше ошибок, исправить нельзя». Такой блок
         * положил бы верные буквы не на своё место, и повторное
         * подтверждение это пропустило бы: оно сверяет знаки,
         * а не их позицию. */
        if ((r0a & R0A_RDSR) && !(r0b & R0B_ABCD_E) &&
            (r0b & R0B_BLERB_MASK) != R0B_BLERB_MASK) {
            uint16_t rb = 0, rd = 0;
