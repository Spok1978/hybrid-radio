 ...
 * пересчитывать на каждый щелчок регулятора значило бы обходить всю
...
static uint32_t rebuild_map(void)
{
    ...
    /* Карта LVGL заканчивается ПУСТОЙ СТРОКОЙ, а не нулевым
     * указателем. Раньше здесь стояло просто src[i] - проверка
     * указателя, - и цикл проезжал признак конца, читая память
     * за массивом. Заполнял мусорными указателями всю карту, после
     * чего lv_buttonmatrix_set_map уходил их разбирать и вешал
     * задачу намертво: подъём интерфейса не доходил до клавиатуры,
     * а сторож ловил голодание простаивающей задачи.
     * Найдено на живой плате 2026-08-30. */
    for (size_t i = 0; src[i] && src[i][0] &&
                       n < (sizeof(s_map) / sizeof(s_map[0])) - 1; i++) {
        const char *k = src[i];
...
static void on_matrix_click(lv_event_t *e)
{
    ...
    /* Касание экрана — не двойное нажатие регулятора. Сбрасываем отсчёт,
...
void ipradio_keyboard_select(void)
{
     ...
     * событие LVGL: нажатие приходит с физической кнопки регулятора,
