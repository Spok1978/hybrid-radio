...
/** Номер поколения содержимого: растёт при каждой удачной записи.
 *
 *  Нужен тому, кто показывает банк на экране. Перечитывать файл
 *  на каждую перерисовку нельзя - это копия в пару килобайт под
 *  замком, десять раз в секунду. Сравнить одно число дёшево,
 *  а меняется банк редко: автопоиском, поиском по каталогу
 *  и долгим нажатием кнопки пресета. */
uint32_t ipradio_storage_generation(void);

/** Положить содержимое в память, НЕ записывая на носитель.
 *
 *  Нужно для проверок на столе, когда карты нет: без неё обычная
 *  запись отказывает целиком, и прибор нечем наполнить — ни станцией,
 *  ни настройкой. Между включениями такое, разумеется, не переживает. */
void ipradio_storage_put_ram(const ipradio_store_t *in);

/* Выборочное чтение. Целая структура банка занимает около 2,5 КБ,
 * и класть её на стек ради одного поля - непозволительно дорого:
 * на этом переполнился стек задачи автомата (4 КБ) при первом же
 * нажатии пресета. Ячейка стоит около 300 байт, настройки - меньше
 * сотни.
 *
 * Полную копию берут только те, кому она правда нужна: перебор
 * ячеек и запись. */

/** Одна ячейка банка, cell = 1…IPRADIO_PRESET_MAX.
 *  Возвращает false, если номер вне диапазона или ячейка пуста. */
