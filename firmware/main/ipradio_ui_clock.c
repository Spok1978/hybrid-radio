...
/* Правка даты и времени. Поле за полем, энкодер меняет значение,
...
static void start_edit(void)
{
     ...
     * полвека энкодером человек не станет. */
    ...
    lv_label_set_text(s_hint,
        "Энкодер 1 — значение   •   нажатие — следующее поле");
    render();
}
...
void ipradio_clock_ui_open(void (*on_close)(void *ctx), void *ctx)
{
    ...
    lv_label_set_text(s_hint,
        "Энкодер 1 — выбор   •   нажатие — изменить   •   "
